...
::  This obfuscator is an edition of dave obfuscator
::  Obfuscation cannot contain characters such as: ./:/!/*/)/(/,/; <between others>
::  The application must be in the same folder as the obfuscator
...
set "find={[:<][^}]*}|^[^:\r\n]?[ \t=,;\xFF]*:[^ \t:\r\n+]*[ \t:\r\n+]?|%%%%|%%\*|%%(?:~[fdpnxsatz]*(?:\$[^:\r\n]+:)?)?[0-9]|%%[^%%\r\n]+%%|%%@[\x20-\x24\x26-\x7E]"
set "repl=$txt=$0@$txt='%%'+String.fromCharCode($0.charCodeAt(0)+129)+'%%'+'%%Obfuscation_Name%%'+'%%Edit_here%%'"
